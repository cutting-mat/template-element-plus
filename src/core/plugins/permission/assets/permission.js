...
export const install = function (app, config) {
  // 路由访问免登录白名单
  ...
  app.config.globalProperties.$Permission = function (authCallback) {
    const checkAccount = (loginCallback) => {
      ...
      if (userToken) {
        ...
        new Promise((resolve) => {
          if (config.AccessControl) {
            resolve(
              AccessControl(app, routeInstance, config.interceptorsRequest)
            );
          } else {
            resolve();
          }
