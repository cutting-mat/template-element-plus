 ...
 * */
const enCrypto = (content: string, secretKey: string) => {
  if (secretKey) {
 ...
 */
const deCrypto = (content: string, secretKey: string) => {
  if (secretKey) {
...
export const CryptoConfig = {
  ...
  GetCryptoUrl: (requestConfig: any) => {
    // 自定义加密请求地址方法
    return `/sapi/${CryptoJS.MD5(requestConfig.url + "|sogdata.com")}`;
  },
  GetSecretKey: (requestConfig: any) => {
    // 自定义加解密密钥方法
    const token =
  ...
  EncryptData: function (requestConfig: any, SecretKey: string) {
    // 自定义加密方法
    const baseParam = {
  ...
  DecryptResponse: function (response: any, SecretKey: string) {
    // 自定义解密方法
